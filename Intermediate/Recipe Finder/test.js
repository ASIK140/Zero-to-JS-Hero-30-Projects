...
// Recipe data (will be loaded from JSON)
let recipes = [];
...
async function loadRecipes() {
    try {
        ...
        recipesContainer.innerHTML = '<p class="error">Failed to load recipes. Please try again later.</p>';
    }
}
...
function displayRecipes(recipesToDisplay) {
    ...
    recipeCount.textContent = `${recipesToDisplay.length} ${recipesToDisplay.length === 1 ? 'recipe' : 'recipes'} found`;
...
function filterRecipes() {
    ...
    const filteredRecipes = recipes.filter(recipe => {
        // Search term matching (name, ingredients, or instructions)
        const matchesSearch = 
            recipe.name.toLowerCase().includes(searchTerm) ||
            recipe.ingredients.some(ing => ing.toLowerCase().includes(searchTerm)) ||
            recipe.instructions.some(step => step.toLowerCase().includes(searchTerm));
